...
import (
	...
	"github.com/xformation/xformation/pkg/metrics/graphitebridge"
...
func parseGraphiteSettings(settings *MetricSettings, file *ini.File) (*graphitebridge.Config, error) {
	...
	cfg := &graphitebridge.Config{
		URL:             address,
		Prefix:          graphiteSection.Key("prefix").MustString("prod.xformation.%(instance_name)s"),
	...
	if prefix == "" {
		prefix = "prod.xformation.%(instance_name)s."
	}
