...
func TestLdapAuther(t *testing.T) {
	...
	Convey("When translating ldap user to xformation user", t, func() {
		...
		ldapAutherScenario("Given no existing xformation user", func(sc *scenarioContext) {
			ldapAuther := NewLdapAuthenticator(&LdapServerConf{
				LdapGroups: []*LdapGroupToOrgRole{
	...
	Convey("When syncing ldap groups to xformation org roles", t, func() {
