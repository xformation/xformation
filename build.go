...
var (
	...
	buildNumber           int      = 0
	binaries              []string = []string{"xformation-server", "xformation-cli"}
)
...
func main() {
	...
	for _, cmd := range flag.Args() {
		...
		case "build-cli":
			clean()
			build("xformation-cli", "./pkg/cmd/xformation-cli", []string{})
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		...
		".rpm":    "dist/xformation-latest-1.x86_64.rpm",
		".tar.gz": "dist/xformation-latest.linux-x64.tar.gz",
	}
...
func createDebPackages() {
	createPackage(linuxPackageOptions{
		packageType:            "deb",
		homeDir:                "/usr/share/xformation",
		binPath:                "/usr/sbin",
		configDir:              "/etc/xformation",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/xformation-server",
		initdScriptFilePath:    "/etc/init.d/xformation-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/xformation-server.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/xformation-server",
		defaultFileSrc: "packaging/deb/default/xformation-server",
		systemdFileSrc: "packaging/deb/systemd/xformation-server.service",
...
func createRpmPackages() {
	createPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/xformation-server",
		initdScriptFilePath:    "/etc/init.d/xformation-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/xformation-server.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/xformation-server",
		defaultFileSrc: "packaging/rpm/sysconfig/xformation-server",
		systemdFileSrc: "packaging/rpm/systemd/xformation-server.service",
...
func createPackage(options linuxPackageOptions) {
	packageRoot, _ := ioutil.TempDir("", "xformation-linux-pack")
...
func setup() {
	...
	runPrint("go", "install", "-v", "./pkg/cmd/xformation-server")
}
...
func clean() {
	rmr("dist")
	rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/xformation", goos, goarch)))
}
