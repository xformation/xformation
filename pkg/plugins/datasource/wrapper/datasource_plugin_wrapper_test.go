...
func TestMappingRowValue(t *testing.T) {
	...
	stringRowValue, _ := dpw.mapRowValue(&datasource.RowValue{Kind: datasource.RowValue_TYPE_STRING, StringValue: "xformation"})
	haveString, ok := stringRowValue.(string)
	if !ok || haveString != "xformation" {
		t.Fatalf("Expected %s, was %s", "xformation", haveString)
	}
