...
var apiUrl = flag.String("apiUrl", "https://xformation.com/api", "api url")
var apiKey = flag.String("apiKey", "", "api key")
var version = ""
var versionRe = regexp.MustCompile(`xformation-(.*)\.(linux|windows)`)
var builds = []build{}
...
func main() {
	...
	postRequest("/xformation/versions", nightly, fmt.Sprintf("Create Release %s", nightly.Version))
	postRequest("/xformation/versions/"+nightly.Version, nightly, fmt.Sprintf("Update Release %s", nightly.Version))

	for _, b := range nightly.Builds {
		postRequest(fmt.Sprintf("/xformation/versions/%s/packages", nightly.Version), b, fmt.Sprintf("Create Build %s %s", b.Os, b.Arch))
		postRequest(fmt.Sprintf("/xformation/versions/%s/packages/%s/%s", nightly.Version, b.Arch, b.Os), b, fmt.Sprintf("Update Build %s %s", b.Os, b.Arch))
	}
}
...
func packageWalker(path string, f os.FileInfo, err error) error {
	...
	builds = append(builds, build{
		Os:     os,
		Arch:   "amd64",
		Url:    "https://s3-us-west-2.amazonaws.com/xformation-releases/master/" + f.Name(),
